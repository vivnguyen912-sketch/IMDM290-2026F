...
public class heart : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            float r = 10f;
            startPosition[i] = new Vector3(r * Random.Range(-3f, 1f), r * Random.Range(-2f, 2f), r * Random.Range(-1f, 3f));        

            r = 3f; // radius of the circle
            // Circular end position
            float sin = Mathf.Sin(i * 4 * Mathf.PI / numSphere);
            float cos = Mathf.Cos(i * 2 * Mathf.PI / numSphere);
            endPosition[i] = new Vector3(Mathf.Sqrt(2f) 
            * sin * sin * sin, 
              -4 * cos * cos * cos 
            - cos * cos +4 * cos,  10f) * 3;
        }
